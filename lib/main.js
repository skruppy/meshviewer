  ...
  return function () {
    function handleData(data) {
      ...
      nodes.forEach( function(node) {
        ...
        if (typeof node.neighbours !== "undefined")
          for(var i = node.neighbours.length - 1; i >= 0; i--)
            if(node.neighbours[i].link.vpn) {
              node.vpn = node.neighbours[i]
              break
            }
      })
      ...
      links.forEach( function (d) {
        ...
        if (d.vpn) {
          if (!d.source.node.flags.gateway)
            d.source.node.vpn = d
          if (!d.target.node.flags.gateway)
            d.target.node.vpn = d
        }
      })
